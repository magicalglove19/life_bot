...
_SP400_URL = "https://en.wikipedia.org/wiki/List_of_S%26P_400_companies"   # 중형주
_SP600_URL = "https://en.wikipedia.org/wiki/List_of_S%26P_600_companies"   # 소형주
_INDEX_URLS = {"sp500": _SP500_URL, "sp400": _SP400_URL, "sp600": _SP600_URL}
# 이름 → 구성 지수. sp1500 = 대·중·소형 합산 (미국 시가총액의 약 90%)
UNIVERSES = {"sp500": ("sp500",), "sp400": ("sp400",), "sp600": ("sp600",),
             "sp1500": ("sp500", "sp400", "sp600")}
INDEX_LABEL = {"sp500": "대형", "sp400": "중형", "sp600": "소형"}
UNIVERSE_TITLE = {"sp500": "S&P 500", "sp400": "S&P 400 (중형)", "sp600": "S&P 600 (소형)",
                  "sp1500": "S&P 1500 (대·중·소형)"}
...
def load_universe(name: str = "sp500", custom_file: str | None = None, limit: int | None = None) -> pd.DataFrame:
    ...
    name: 'sp500' | 'sp400' | 'sp600' | 'sp1500' (또는 custom_file 로 직접 목록 지정)
    반환 DataFrame 에는 소속 지수를 뜻하는 'index' 컬럼이 붙는다.
    """
    if custom_file:
        rows = []
        ...
        if name not in UNIVERSES:
            raise ValueError(f"알 수 없는 유니버스: {name} (가능: {', '.join(UNIVERSES)})")
        rows = []
        for idx_name in UNIVERSES[name]:
            part = _load_cache(idx_name)
            if part is None:
                part = _read_wiki_table(_INDEX_URLS[idx_name], "Symbol", "Security", "GICS Sector")
                _save_cache(idx_name, part)
            for r in part:
                rows.append({**r, "index": idx_name})   # 어느 지수 소속인지 (대형/중형/소형)
